    ...
    def start_getImages_thread(self):
        ...
        with device.start_stream(1):
            ...
            while self.getImages_is_running:
                ...
                t2 += time.time()
                folder_path = '/ssd_2tb/lucid_image/{}/{}/jpg/'.format(path_time[:8],self.ip)
                if not os.path.exists(folder_path):
                    os.makedirs(folder_path)
                cv2.imwrite( os.path.join(folder_path,f'{path_time}.jpg'), nparray_reshaped)
                # if count % 4 == 1:
            ...
            with device.start_stream(1):
                ...
                while self.preview_is_running:
                    ...
                    if count % 10 == 1:
                        ...
                        # png_array = PIL_Image.fromarray(nparray_reshaped)
                        # png_array.save(path, quality = 95) # quality high the size is big, max 100
                        with open( os.path.join(folder_path,f'{path_time}.jpg'), "rb") as image_file:  
